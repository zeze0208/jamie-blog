...
> **잠깐 닫아도 되는 사이트**와 **절대 닫으면 안 되는 서비스**가 한 서버에 있었다.
...
서버 복구작업을 해야했던 데에는 완전히 다른 이유가 있었다. 서버 관리자(root) 계정 접근이 어려웠기 때문이다. root 권한이 없으니, 계정 복구를 하기 전까지는 조치할 수 있는 게 거의 없었다. 결국 모든 서비스를 잠시 내리는 선택을 해야만 했다.
...
하루를 꼬박 쓴 일의 결론이 "안 쓰는 게시판 하나를 안 지워서"였다는 게 아직도 좀 허탈하다.


그래도 배운 건 있다. 모르는 영역이라고 손 놓고 기다리는 게 제일 비싼 선택이라는 것. 명령어 한 줄 몰라도 물어보면서 가면 된다. 다만 지우기 전에 개수를 세고, 백업을 먼저 하고, 되돌리기 쉬운 것부터.


안 쓰는 게시판, 지금 한번 확인해보시길.
