...
개발 일정 때문에 못 할 뻔했던 프로모션이 바로 진행될 수 있게 됐으니까. 나는 코드를 이해하게 된 게 아니다. 하지만 일이 진행되게 만들었다. 비개발자에게는 그걸로 충분했다.
...
에러들의 공통점은 하나였다. 클로드코드는 코드를 잘 고쳤다. 나를 막은 건 코드가 아니라, 팀의 마이그레이션 방식, 도구 설치 위치, 운영용 서버 설정, 깃에서 제외된 런타임 파일 같은 시스템의 규칙들이었다. 클로드는 이것들을 몰랐고, 알 방법도 없었다. 코드 어디에도 적혀 있지 않았으니까.


에러는 AI가 틀려서 나는 게 아니라, AI가 우리 시스템을 몰라서 난다. 이 한 문장을 이해하고 나니 다음에 뭘 준비해야 하는지가 분명해졌다.


**2. 가장 강력한 프롬프트는 잘 쓴 문장이 아니라 '시스템 문서'다**


결과를 가르는 건 프롬프트 문장의 기교가 아니라 맥락을 얼마나 줬는가였다. 서버가 운영용 설정으로 돈다는 한 줄, 마이그레이션 도구 위치 한 줄, 깃에 없는 파일은 서버에서 직접 만들어야 한다는 한 줄 — 이 몇 줄이 있었다면 이틀이 아니라 반나절로 끝났을지 모른다.


흩어진 맥락을 정리해 문서로 만드는 건 기획자가 가장 잘하는 일이기도 하다. 기록은 다음 작업의 출발선을 앞당긴다.


**3. 비개발자의 무기는 코드 실력이 아니라 '안전하게 끝까지 가는 판단'이다**


코드를 짠 건 클로드였다. 내가 한 건 따로 있었다. 테스트 서버에서만 작업한 것, 되돌릴 수 없는 일은 시작하지 않은 것, 운영 반영을 피곤한 밤이 아닌 다음 날로 미룬 것, 수많은 에러 앞에서 포기하지 않은 것. 리스크 관리, 질문으로 좁혀 가기, 일이 굴러가게 만들기 — 기획자가 평소에 하던 일과 다르지 않다. 코드는 클로드가 쓴다. 판단은 내가 한다.
...
- 본서버 말고 테스트 서버부터. 복구 못 할 일은 시작하지 않는다.
- 팀의 기존 방식을 AI에게 먼저 알려준다. 이게 에러의 최대 원인이다.
- 시스템 규칙을 적은 문서가 최고의 프롬프트다.
- 모르는 용어는 그냥 물어본다. 다 이해 못해도 일은 진행된다.
- 빨간 글씨라고 다 멈출 일은 아니다. AI에게 물어 구분한다.
