...
import { CATEGORY_LABELS, FIXED_CATEGORIES } from '../../utils/categories';
import '../post-tabs/style.scss';

// 사이드바에 노출할 태그 최대 개수
const MAX_SIDEBAR_TAGS = 20;

// 글 상세/태그 페이지 좌측에 카테고리/태그 사이드바를 보여준다 (홈 화면의 post-tabs 사이드바와 동일한 스타일을 재사용).
// 홈과 달리 여기서는 클릭 시 실제 카테고리/태그 페이지로 이동한다 (탭 전환 상태가 없음).
function PostSidebar({ posts, activeCategories = [], activeTags = [] }) {
  // 카테고리 순서는 항상 FIXED_CATEGORIES 기준 고정 (글 최신순으로 즉석 계산하면 순서가 계속 바뀜)
          ...
          <ul className="sidebar-tag-list">
            {tagCounts.map(([tag, count]) => (
              <li key={tag}>
                <Link
                  to={`/tags/${tag}`}
                  className={`sidebar-tag ${activeTags.includes(tag) ? 'selected' : ''}`}
                >
